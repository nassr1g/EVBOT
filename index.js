...
function buildStatusText(groupInfo, groupState, lastAiLatencyMs, botStartedAt) {
  const lines = []
  lines.push('┏━━━━━━━━━━━━━━━━━━━')
...
async function startBot() {
  ...
  sock.ev.on('connection.update', async ({ connection, lastDisconnect, qr }) => {
    ...
    if (connection === 'close') {
      ...
      console.log(`🔌 الاتصال انقطع، رمز الحالة: ${code} (515 = يتطلب إعادة تشغيل، شائع بعد أول ربط جديد)`)
      ...
      // مهم: نزيل كل مستمعي هذا الـsock القديم قبل إعادة التشغيل. بدون هذا، لو حصل انقطاع
      // "restart required" (شائع فعلياً بعد أول ربط جديد بالضبط — موثّق برمز 515 وبعلامة
      // isNewLogin بمكتبة Baileys نفسها)، تبقى دالة الاستماع القديمة مسجّلة وتحمل بداخلها
      // botJid/botLid فارغين للأبد (لأن هذا الـsock بالذات ما وصل أبداً لحالة 'open' الحقيقية
      // قبل ما يُطلب إعادة التشغيل)، فتستمر تعالج رسائل جديدة بمعرّف بوت فارغ فيفشل فحص
      // المنشن/الرد بشكل دائم — وهذا بالضبط ما يفسر بقاء العطل حتى بعد دقائق من الاتصال
  ...
  sock.ev.on('messages.upsert', async ({ messages, type }) => {
    ...
    const addressResult = detectBotAddress(text || '', selfIds, msg)
    ...
    if (isExecutableCommand) {
      ...
      switch (classified.action) {
        ...
        case 'delete_quoted': reactionEmoji = "🗑️"; break;
    ...
    await handleClassifiedAction({
      ...
      lastAiLatencyMs, botStartedAt, senderBare,
    })
  })
}
...
async function handleClassifiedAction({
  sock, chatId, msg, text, classified, groupInfo, groupState,
  senderIsAdmin, isGroup, hasImage, reply, selfIds, lastAiLatencyMs, botStartedAt, senderBare,
}) {
  switch (classified.action) {
    case 'help': {
    ...
    case 'status': {
      const statusText = buildStatusText(groupInfo, groupState, lastAiLatencyMs, botStartedAt)
