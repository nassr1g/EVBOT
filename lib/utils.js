...
function detectBotAddress(text, selfIds, msg) {
  ...
  if (ctx) {
    ...
    // ── تشخيص مؤقت: نطبع كل شي لو ما انطابق أي فحص رغم وجود ctx (رد أو منشن) ──
    console.log('🔍 [تشخيص مؤقت] detectBotAddress فشل رغم وجود ctx')
    console.log('🔍 [تشخيص مؤقت] selfIds =', JSON.stringify(selfIds))
    console.log('🔍 [تشخيص مؤقت] ctx.participant =', ctx.participant)
    console.log('🔍 [تشخيص مؤقت] ctx.participantPn =', ctx.participantPn)
    console.log('🔍 [تشخيص مؤقت] ctx.remoteJid =', ctx.remoteJid)
    console.log('🔍 [تشخيص مؤقت] ctx.mentionedJid =', JSON.stringify(ctx.mentionedJid))
    console.log('🔍 [تشخيص مؤقت] replyIds بعد bareId =', JSON.stringify(replyIds))
    console.log('🔍 [تشخيص مؤقت] mentionIds بعد bareId =', JSON.stringify(mentionIds))
  } else {
    console.log('🔍 [تشخيص مؤقت] detectBotAddress: لا يوجد ctx إطلاقاً بهذه الرسالة')
  }
