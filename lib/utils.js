 ...
 *
 * @param {object} sock - اختياري، يُستخدم فقط كخط دفاع أخير لحل LID الرد/المنشن الوارد إلى
 *   رقم هاتف عبر sock.signalRepository.lidMapping.getPNForLID، إذا فشلت المطابقة المباشرة.
 *   هذا اتجاه معاكس لمحاولات سابقة كانت تحاول تحديد LID الخاص بالبوت نفسه مسبقاً (فشلت لأن
 *   واتساب لا يُعطي هذي المعلومة أحياناً لحسابات معينة)؛ هنا بدلاً من ذلك نسأل عن هوية
 *   الطرف الآخر المرسل نفسه، وهذا الاستعلام أثبت نجاحه بحالات كثيرة حتى لو فشل الاتجاه الأول
 * @returns {Promise<{ called: boolean, isGenericBotWord: boolean, triggeredBy: 'text'|'mention'|'reply'|null }>}
 ...
 */
async function detectBotAddress(text, selfIds, msg, sock = null) {
  ...
  const ctx = getContextInfo(msg)
  if (ctx) {
    const replyIds = [ctx.participant, ctx.participantPn, ctx.participantAlt]
      .filter(Boolean)
      .map(bareId)
    ...
    // خط دفاع أخير: لو عندنا sock ووصل معرّف LID لم يطابق مباشرة، نسأل Baileys يحوّل هذا
    // LID بالذات لرقم هاتف (اتجاه معاكس لمحاولة معرفة LID بوتنا نفسه، والتي فشلت سابقاً)
    if (sock?.signalRepository?.lidMapping?.getPNForLID) {
      const rawCandidates = [ctx.participant, ...(ctx.mentionedJid || [])].filter(Boolean)
      const lidCandidates = rawCandidates.filter(id => String(id).includes('@lid'))

      for (const lidJid of lidCandidates) {
        try {
          const resolved = await sock.signalRepository.lidMapping.getPNForLID(lidJid)
          const resolvedValue = typeof resolved === 'string' ? resolved : (resolved?.pn || resolved?.jid || '')
          if (resolvedValue && selfIds.includes(bareId(resolvedValue))) {
            const triggeredBy = lidJid === ctx.participant ? 'reply' : 'mention'
            console.log(`✅ تطابق عبر getPNForLID: ${lidJid} → ${resolvedValue}`)
            return { called: true, isGenericBotWord: false, triggeredBy }
          }
        } catch (e) {
          console.error('⚠️ فشل getPNForLID لـ', lidJid, ':', e.message)
        }
      }
    }

    // تشخيص شامل: منشن أو رد موجود بالرسالة لكن ما طابق selfIds حتى بعد كل المحاولات.
    // واتساب يستخدم حالياً نوعين من المعرّفات (LID الجديد وPN رقم الهاتف التقليدي)، وهما
    // فضاءا أرقام مختلفان تماماً. نطبع كل القيم الخام هنا لمعرفة الصيغة الفعلية المستخدمة
    console.log('🔍 تشخيص مطابقة المنشن/الرد (فشلت كل المحاولات):')
    console.log('   selfIds =', JSON.stringify(selfIds))
    console.log('   ctx.participant =', ctx.participant)
    console.log('   ctx.participantPn =', ctx.participantPn)
    console.log('   ctx.participantAlt =', ctx.participantAlt)
    console.log('   ctx.mentionedJid =', JSON.stringify(ctx.mentionedJid))
    console.log('   replyIds (بعد bareId) =', JSON.stringify(replyIds))
    console.log('   mentionIds (بعد bareId) =', JSON.stringify(mentionIds))
  }

  return { called: false, isGenericBotWord: false, triggeredBy: null }
}

// يبحث عن LID الخاص بالبوت نفسه داخل قائمة أعضاء مجموعة (المُجلَبة أصلاً لفحوصات الإشراف
// بأي حال)، بدل الاعتماد على sock.user.lid أو مخزن LID الداخلي بمكتبة Baileys (ثبت أن
// كليهما قد لا يكونا موثوقين بعد ربط جديد). نجد مدخل عضويتنا نحن بمطابقة رقم هاتفنا
// المعروف أصلاً (botJid، يعمل دائماً بثقة)، ثم نقرأ من نفس ذلك المدخل حقل الـLID الخاص به
// مباشرة — بيانات جماعية حقيقية وحديثة، لا ذاكرة تخزين مؤقت داخلية قد تكون فارغة
function findOwnLidFromParticipants(participants, botJid) {
  if (!Array.isArray(participants) || !botJid) return ''
  const botBare = bareId(botJid)

  for (const p of participants) {
    // نجرب كل الحقول المحتملة لتمثيل رقم الهاتف على مستوى إصدارات Baileys المختلفة
    const candidatePnFields = [p.id, p.jid, p.pn, p.phoneNumber].filter(Boolean)
    const isUs = candidatePnFields.some(v => bareId(v) === botBare)
    if (!isUs) continue

    // وجدنا أنفسنا — الآن نقرأ الـLID من نفس المدخل، بأي حقل متاح يحمله. القيمة تُعاد
    // كما هي (نص خام)؛ كل من يستدعي هذي الدالة يمرر الناتج عبر bareId() على أي حال
    if (p.lid) return String(p.lid).trim()
    // بعض الإصدارات تضع الـLID مباشرة بحقل id نفسه (لو كان الحساب من النوع الذي id فيه LID)
    if (p.id && String(p.id).includes('@lid')) return String(p.id).trim()
  }
...
async function isBotMentionedOrReplied(text, selfIds, msg, sock = null) {
  const result = await detectBotAddress(text, selfIds, msg, sock)
...
// شريط عرض نسبة استهلاك حصة (Gemini/Tavily/Serper) — نفس شكل renderProgressBar بصرياً،
// لكن بدلالة مختلفة تماماً (نسبة استهلاك تراكمية بطيئة التغير، لا تقدم تحميل لحظي)، لذا
// دالة منفصلة بدل إعادة استخدام renderProgressBar مباشرة، حتى يبقى القصد واضحاً بالكود
function renderUsageBar(percentUsed, width = 12) {
